    ...
    for i, v in enumerate(db):
        ...
        for j, t in tracks.items():
            if i in t.keys():
                ...
                ax.annotate(f'{t_i[4]:9.2f}', (t_i[0] + (t_i[2] - t_i[0]) / 2.0, t_i[1] + (t_i[3] - t_i[1]) / 2.0),
...
def save_evaluate_mot_accums(accums, names, output_file, generate_overall=False):
    mh = mm.metrics.create()
    summary = mh.compute_many(
        accums,
        metrics=mm.metrics.motchallenge_metrics,
